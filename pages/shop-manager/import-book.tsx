...
//
//   useEffect(() => {
//     setTotalCost(Array.from(value.values()).reduce((accumulator, currentValue) => accumulator + (currentValue.price * currentValue.quantity), 0));
//   });
//
//   return { value, setValue, totalCost, setTotalCost };
// };
//
// function Cashier(props: { token: TokenType }) {
...
//     const locationDetail = data['location'].find((location: { [x: string]: string; }) => location['buildingId'] == props.token.assignedBuilding);
//
//     if (!locationDetail) {
...
//   return <>{
//     Array.from(props.books.values()).map(
//       (row, index) =>
...
function ShopManager2() {
  return <>Something here</>;
}

export default ShopManager2;
